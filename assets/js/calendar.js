...
class CalendarManager {
  ...
  async init() {
    await Promise.all([this.fetchRange(), this.loadMood(), window.loadFearGreed?.()]);
    this.renderPicker();
    this.initCalendarAlerts();
  }
  ...
  // ── MACRO EVENT ALERTS ──────────────────────────────────
  initCalendarAlerts() {
    const cfg = JSON.parse(localStorage.getItem('mp-calendar-alerts') || '{"enabled":false,"minutes_before":60}');
    const en = document.getElementById('calAlertEnabled');
    const mins = document.getElementById('calAlertMins');
    if (en) en.checked = !!cfg.enabled;
    if (mins) mins.value = cfg.minutes_before || 60;
  }

  saveCalendarAlerts() {
    const cfg = {
      enabled: !!document.getElementById('calAlertEnabled')?.checked,
      keywords: ['FOMC', 'CPI', 'NFP', 'Fed', 'ECB'],
      minutes_before: parseInt(document.getElementById('calAlertMins')?.value || '60', 10),
    };
    localStorage.setItem('mp-calendar-alerts', JSON.stringify(cfg));
    window.authManager?.scheduleSync();
    Toast.show(cfg.enabled ? 'Macro alerts on (~1h before)' : 'Macro alerts off', 'success');
  }
  ...
  exportCSV() {
    ...
    const rows = window.MpExport.calendarRows(this.events);
    window.MpExport.downloadExcel(rows, `economic_calendar_${stamp}.xlsx`, {
      sheetName: 'Calendar',
      dateCols: [0],
      colWidths: [14, 12, 42, 18, 12, 12, 12, 12],
    });
  }
